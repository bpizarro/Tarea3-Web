...
import {
  ...
  buildExportContent,
  type ExportFormat,
} from '@domain/usecases/SurveyUseCases'
...
export function useAdminEntries(): AdminEntriesState & AdminEntriesActions {
  ...
  /**
   * Exporta las entradas escribiendo un archivo temporal en el sistema de archivos
   * del dispositivo y luego abriendo el diálogo nativo de compartir (expo-sharing).
   *
   * Flujo:
   * 1. buildExportContent genera el string del archivo (dominio puro)
   * 2. FileSystem.writeAsStringAsync escribe el archivo en caché del app
   * 3. Sharing.shareAsync abre el sheet nativo para guardar/enviar
   */
  const exportEntries = useCallback(async (format: ExportFormat) => {
    ...
    try {
      ...
      // Móvil: usa el sheet nativo para compartir/guardar con el sistema
      const file = new File(Paths.cache, filename)
      file.write(content, { encoding: 'utf8' })

      // Verifica que el dispositivo soporte compartir antes de invocar
