...
import type { FormData } from '@usecases/SurveyUseCases'
...
export function SurveyForm() {
	...
	useEffect(() => {
		if (!isSuccess) return
		const timer = setTimeout(resetSuccess, 5000)
				...
				>
					<View style={styles.cardHeader}>
						...
						<View style={styles.flex}>
							<Text style={[styles.cardTitle, { color: colors.tp }]}>Encuesta & Solicitud de Servicio</Text>
							<Text style={[styles.cardSub, { color: colors.ts }]}>Cuéntanos sobre ti y cómo podemos ayudarte.</Text>
						</View>
					</View>

					<View style={[styles.dateBadge, { backgroundColor: colors.as, borderColor: colors.bd }]}> 
						<Text style={styles.dateIcon}>📅</Text>
						<Text style={[styles.dateText, { color: colors.ac }]} numberOfLines={1}>{dateLabel}</Text>
					</View>
					...
					<FormField
						...
						onChangeText={(value) => handleChange('company' as keyof FormData, value)}
					...
					{isSuccess ? <SuccessBanner onDismiss={resetSuccess} /> : null}
