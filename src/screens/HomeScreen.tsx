				...
				<View style={[styles.main, isCompact && styles.mainCompact]}>
					...
					<View style={[styles.hero, { borderBottomColor: colors.bd }]}> 
						<Text style={[styles.heroTitle, { color: colors.tp }]}> 
							{activeTab === 'responses' && isAdmin ? 'Respuestas recibidas' : 'Tu opinión nos importa'}
						</Text>
					...
					{isAdmin && (
						<View style={[styles.tabBar, { borderBottomColor: colors.bd, backgroundColor: colors.sur }]}> 
							...
							>
								<Text style={[styles.tabText, { color: activeTab === 'form' ? colors.ac : colors.tm }]}>📋 Formulario</Text>
							</TouchableOpacity>
							<TouchableOpacity
								style={[styles.tab, activeTab === 'responses' && { borderBottomColor: colors.ac }]}
								onPress={() => setActiveTab('responses')}
							>
								<Text style={[styles.tabText, { color: activeTab === 'responses' ? colors.ac : colors.tm }]}>📊 Respuestas</Text>
							</TouchableOpacity>
						</View>
					)}

					{isAdmin && activeTab === 'responses' ? <AdminPanel /> : <SurveyForm />}

					<View style={[styles.footer, { borderTopColor: colors.bd }]}> 
						<Text style={[styles.footerText, { color: colors.tm }]}>SurveyForm · Datos almacenados en tu dispositivo · 2026</Text>
					</View>
				</View>
			</View>
		</SafeAreaProvider>
...
function AdminPanel() {
	...
	const confirmDelete = (entry: SurveyEntry) => {
		Alert.alert('Eliminar respuesta', `¿Eliminar la respuesta de ${entry.fullName}?`, [
	...
	return (
		<View style={[styles.flex, styles.adminPanel]}>
			<View style={[styles.adminBar, isCompact && styles.adminBarCompact, { backgroundColor: colors.sur, borderColor: colors.bd }]}> 
				<View style={styles.adminBarLeft}>
					<Text style={styles.shieldIcon}>🛡️</Text>
					<Text style={[styles.adminBarTitle, { color: colors.tp }]}>Panel Admin</Text>
					<Text style={[styles.countText, { color: colors.tm }]}>· {entries.length} respuesta{entries.length !== 1 ? 's' : ''}</Text>
				</View>
				<View style={[styles.adminBarActions, isCompact && styles.adminBarActionsCompact]}>
					<TouchableOpacity style={[styles.dlBtn, { backgroundColor: colors.as, borderColor: colors.bd }]} onPress={() => void exportEntries('csv')} disabled={isExporting || entries.length === 0}>
						<Text style={[styles.dlBtnText, { color: colors.ac }]}>CSV</Text>
					</TouchableOpacity>
					<TouchableOpacity style={[styles.dlBtn, { backgroundColor: colors.as, borderColor: colors.bd }]} onPress={() => void exportEntries('json')} disabled={isExporting || entries.length === 0}>
						<Text style={[styles.dlBtnText, { color: colors.ac }]}>JSON</Text>
					</TouchableOpacity>
				</View>
			</View>

			{error ? <Text style={[styles.errorText, { color: colors.er }]}>{error}</Text> : null}

			{entries.length === 0 ? (
				<View style={styles.emptyState}>
					<Text style={styles.emptyIcon}>📭</Text>
					<Text style={[styles.emptyText, { color: colors.tm }]}>Aún no hay respuestas guardadas.</Text>
				</View>
			) : (
				<FlatList
					data={entries}
					...
					renderItem={({ item }) => <EntryCard entry={item} onDelete={() => confirmDelete(item)} />}
				/>
			)}
		</View>
	)
		...
		<View style={[styles.entryCard, { backgroundColor: colors.sur, borderColor: colors.bd }]}> 
			<View style={styles.entryTop}>
				...
				<View style={styles.flex}>
					<Text style={[styles.entryName, { color: colors.tp }]} numberOfLines={1}>{entry.fullName}</Text>
					<Text style={[styles.entryEmail, { color: colors.ts }]} numberOfLines={1}>{entry.email}</Text>
				</View>
...
const styles = StyleSheet.create({
	...
	entryEmail: { fontSize: fontSizes.sm },
